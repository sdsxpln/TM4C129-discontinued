...
#include "FreeRTOS_tasks/display_task.h"
...
#define CMD0_SOFTWARE_RESET				( 0x40+0 )	/* GO_IDLE_STATE */
#define CMD1_START_INITIALISE_MMC		( 0x40+1 )	/* SEND_OP_COND (MMC) */
#define	CMD41_START_INIT_SDC			( 0xC0+41 )	/* SEND_OP_COND (SDC) */
#define CMD8_CHECK_VOLTAGE_RANGE	    ( 0x40+8 )	/* SEND_IF_COND */
#define CMD9_READ_CSD_REGISTER			( 0x40+9 )	/* SEND_CSD */
#define CMD10_READ_CID_REGISTER			( 0x40+10 )	/* SEND_CID */
#define CMD12_STOP						( 0x40+12 )	/* STOP_TRANSMISSION */
#define CMD13_READ_SD_STATUS			( 0xC0+13 )	/* SD_STATUS (SDC) */
#define CMD16_CHANGE_RW_BLOCK_SIZE		( 0x40+16 )	/* SET_BLOCKLEN */
#define CMD17_READ_SINGLE_BLOCK			( 0x40+17 )	/* READ_SINGLE_BLOCK */
#define CMD18_READ_MULTI_BLOCK			( 0x40+18 )	/* READ_MULTIPLE_BLOCK */
#define CMD23_SET_BLOCK_COUNT			( 0x40+23 )	/* SET_BLOCK_COUNT (MMC) */
#define	CMD23_SET_ERASE_COUNT			( 0xC0+23 )	/* SET_WR_BLK_ERASE_COUNT (SDC) */
#define CMD24_WRITE_SINGLE_BLOCK	    ( 0x40+24 )	/* WRITE_BLOCK */
#define CMD25_WRITE_MULTI_BLOCK			( 0x40+25 )	/* WRITE_MULTIPLE_BLOCK */
#define CMD55_LEADING_COMMAND_OF_ACMD	( 0x40+55 )	/* APP_CMD */
#define CMD58_READ_OCR					( 0x40+58 )	/* READ_OCR */
...
const portTickType xTimeOut = 1000UL / portTICK_RATE_MS;
	...
	else if( ( xDiskStatus & STA_NOINIT ) != 0U )
	{
		...
		cCardType = 0;
		if( prvSendCommand( CMD0_SOFTWARE_RESET, 0 ) == 1 )
		{
			/* Enter Idle state.  Initialization timeout of 1000 msec */
			xTimeAtStart = xTaskGetTickCount();

			if( prvSendCommand( CMD8_CHECK_VOLTAGE_RANGE, 0x1AA ) == 1 )
			{
				/* SDHC */
				...
				if( ( ucBuffer[ 2 ] == 0x01 ) && ( ucBuffer[ 3 ] == 0xAA ) )
				{
					...
					/* Wait for leaving idle state (CMD41_START_INIT_SDC
					with HCS bit) */
					while( ( ( xTaskGetTickCount() - xTimeAtStart ) < xTimeOut ) && ( prvSendCommand( CMD41_START_INIT_SDC, 1UL << 30 ) != 0 ) )
					{
						/* Nothing to do here. */
					}

					if( ( ( xTaskGetTickCount() - xTimeAtStart ) < xTimeOut ) && ( prvSendCommand( CMD58_READ_OCR, 0 ) == 0 ) )
					{
			...
			{
				/* SDSC or MMC */
				if( prvSendCommand( CMD41_START_INIT_SDC, 0 ) <= 1 )
				{
					/* SDv1 */
					cCardType = CT_SD1;
					cCommand = CMD41_START_INIT_SDC;
				}
				else
				{
					/* MMCv3 */
					cCardType = CT_MMC;
					cCommand = CMD1_START_INITIALISE_MMC;
				}
				...
				if( ( ( xTaskGetTickCount() - xTimeAtStart ) >= xTimeOut ) || ( prvSendCommand( CMD16_CHANGE_RW_BLOCK_SIZE, mmcSECTOR_SIZE ) != 0 ) )
				{
    ...
    if(xReturn) {
        ...
        sprintf( debug_string, "mmc.c disk_init failed with xReturn: %d", xReturn);
        DisplayString(16, 152 , debug_string);
    } else {
        DisplayString(16, 152 , "disk_init success");
    }
...
DRESULT xReturn;
	...
	else
	{
		...
		{
			/* Single block read */
			if( prvSendCommand( CMD17_READ_SINGLE_BLOCK, ulSector ) == 0 )
			{
		...
		{
			/* Multiple block read */
			if( prvSendCommand( CMD18_READ_MULTI_BLOCK, ulSector ) == 0 )
			{
				do
				{
...
DRESULT xReturn;
	...
	else
	{
		...
		{
			/* Single block write */
			if( prvSendCommand( CMD24_WRITE_SINGLE_BLOCK, ulSector ) == 0 )
			{
		...
		else
		{
			...
			{
				prvSendCommand( CMD23_SET_ERASE_COUNT, xCount );
			}

			if( prvSendCommand( CMD25_WRITE_MULTI_BLOCK, ulSector ) == 0 )
			{
				do
				{
    ...
    if(xReturn) {
        char debug_string[ 64 ];
        sprintf( debug_string, "mmc.c disk_write failed with xReturn: %d", xReturn);
        DisplayString(16, 164 , debug_string);
    } else {
        DisplayString(16, 164 , "disk_write success");
    }
			...
			else
			{
				switch( cControlCode )
				{
					...
					case GET_SECTOR_COUNT:
						...
						if( prvQueryCard( CMD9_READ_CSD_REGISTER, 0, pcTempBuffer, 16 ) == TRUE )
						{
					...
					case GET_BLOCK_SIZE:
						...
						{
							/* SDC ver 2.00 */
							if( prvQueryCard( CMD13_READ_SD_STATUS, 0, pcTempBuffer, mmcMAX_QUERY_STRING_BYTES ) == TRUE )
							{
					...
					case MMC_GET_CSD:
						...
						if( prvQueryCard( CMD9_READ_CSD_REGISTER, 0, pcExternalBuffer, 16 ) == TRUE )
						{
							xResult = RES_OK;
						}
						break;
					...
					case MMC_GET_CID:
						...
						if( prvQueryCard( CMD10_READ_CID_REGISTER, 0, pcExternalBuffer, 16 ) == TRUE )
						{
							xResult = RES_OK;
						}
						break;
					...
					case MMC_GET_OCR:
						...
						if( prvSendCommand( CMD58_READ_OCR, 0 ) == 0 )
						{
							/* READ_OCR */
					...
					case MMC_GET_SDSTAT:
						...
						if( prvQueryCard( CMD13_READ_SD_STATUS, 0, pcExternalBuffer, 64 ) == TRUE )
						{
							xResult = RES_OK;
						}
...
static BYTE prvSendCommand( BYTE cCommand,	DWORD xArgument	)
{
    DisplayString(16, 164 , "prvSendCommand entered");
...
BYTE cCommandString[ mmcCOMMAND_LENGTH_BYTES ];
	...
	if( ( cCommand & 0x80 ) != 0 )
	{
		/* ACMD<n> is the command sequence of CMD55_LEADING_COMMAND_OF_ACMD-CMD<n> */
		cCommand &= 0x7F;
		cResult = prvSendCommand( CMD55_LEADING_COMMAND_OF_ACMD, 0 );
	}
	...
	if( cResult <= 1 )
	{
		...
		if( prvSelectCard() == TRUE )
		{
            DisplayString(16, 164 , "prvSendCommand select success");
			/* Send command packet */
			...
			if( cCommand == CMD0_SOFTWARE_RESET )
			{
				/* Valid CRC for CMD0_SOFTWARE_RESET(0) */
				cCommandString[ 5 ] = 0x95;
			}
			else if( cCommand == CMD8_CHECK_VOLTAGE_RANGE )
			{
				/* Valid CRC for CMD8_CHECK_VOLTAGE_RANGE(0x1AA) */
            ...
            DisplayString(16, 164 , "prvSendCommand select failed");
        }
	}
...
static BOOL prvReceiveDataBlock( BYTE *pcBuffer, UINT xBytesToRead )
{
    ...
    DisplayString(16, 176 , "prvRecieveDataBlock entered");
...
const portTickType xMaxTimeToWait_ms = 200 / portTICK_RATE_MS;
	...
	if( *pcToken == 0xfe )
	{
        ...
        DisplayString(16, 176 , "prvRecieveDataBlock success");
	} 
    else {
        DisplayString(16, 176 , "prvRecieveDataBlock pcToken incorrect");
    }
	return xReturn;
}
/*-----------------------------------------------------------*/
